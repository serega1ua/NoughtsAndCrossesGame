...
// для имитации передачи от Back End массива со значениями
...
var game = {
    ...
    init: function(rowsNumber, cellsNumber) {
        var rowsNumberFinal = rowsNumber || 3;
        var cellsNumberFinal = cellsNumber || 3;
        if (rowsNumberFinal > cellsNumberFinal ) cellsNumberFinal  = rowsNumberFinal;
        if (rowsNumberFinal < cellsNumberFinal ) rowsNumberFinal = cellsNumberFinal ;
        for (var i = 0; i < rowsNumberFinal; i++) {
            this.cells[i] = [];
            for (var j = 0; j < cellsNumberFinal ; j++) {
                this.cells[i].push(this.createElement(i, j));
            }
        }
    },
    ...
    //             this.cells[i][j].style.backgroundImage = this.backgrounder1;
    ...
    //                 this.cells[i][j].onclick = function(e) {
    ...
    //                 this.cells[i][j].style.backgroundPosition = "center center";
    //                 this.cells[i][j].style.backgroundRepeat = "no-repeat";
    //                 this.cells[i][j].style.backgroundImage = this.backgrounder2;
    //                 this.cells[i][j].innerText = "dataset.value = " + this.cells[i][j].dataset.value;
    //                 this.cells[i][j].innerText += "\r get from BE \r" + "cells[" + this.cells[i][j].dataset.i + "][" + this.cells[i][j].dataset.j + "]" + "\r";
    ...
    //                 this.cells[i][j].style.backgroundPosition = "center center";
    //                 this.cells[i][j].style.backgroundRepeat = "no-repeat";
    //                 this.cells[i][j].style.backgroundImage = this.backgrounder3;
    //                 this.cells[i][j].innerText = "dataset.value = " + this.cells[i][j].dataset.value;
    //                 this.cells[i][j].innerText += "\r get from BE \r" + "cells[" + this.cells[i][j].dataset.i + "][" + this.cells[i][j].dataset.j + "]" + "\r";
    //                 this.cells[i][j].onclick = false;
    ...
    //     if (numberOfNullInArrayFromBE === this.cells.length * this.cells[0].length) {
    //         this.turnerState = undefined;
    //         console.log("очередь хода крестиком");
    //     }
    //     if (numberOfTrueInArrayFromBE === numberOfFalseInArrayFromBE) {
    //         this.turnerState = true;
    //         console.log("очередь хода крестиком");
    //     }
    //     if (numberOfTrueInArrayFromBE < numberOfFalseInArrayFromBE) {
    ...
    //     if (numberOfTrueInArrayFromBE > numberOfFalseInArrayFromBE) {
    //         this.turnerState = false;
    //         console.log("очередь хода ноликом");
    //     }
    //     this.searchOfWinner();
    // },
    ...
    createElement: function(i, j) {
        ...
        //element.style.backgroundImage = this.backgrounder4;
        ...
        element.innerText = "cells[" + i + "][" + j + "]" + "\r";
        element.innerText += "i=" + i + "\n";
        element.innerText += "j=" + j + "\n";
        element.innerText += "тут row " + j + "\n";
        element.innerText += "тут cell " + i + "\r";
    ...
    renderElement: function() {
        ...
        for (var i = 0, len = this.cells.length; i < len; i++) {
            //cells[i] - это массив, вложенный в ячейку основного массива
            //if (j == 0) {        element.style.clear = "both";        }
            for (var j = 0, len2 = this.cells[i].length; j < len2; j++) {
                wrapper1.appendChild(this.cells[i][j]);
                this.cells[i][j].onclick = function(e) {
                    game.handleOfCellClick(e.target);
                };
            }
      ...
      //идем 4 шага от i=0 до i=3 в пределах cells.length (это 4)
      //   for (var i = 0; i < this.cells.length; i++) {

            //cells[j] - это массив, вложенный в ячейку основного массива
            // Array(4)
            // 0 : (4) [div.cellClass1, div.cellClass1, div.cellClass1, div.cellClass1]
            // 1 : (4) [div.cellClass1, div.cellClass1, div.cellClass1, div.cellClass1]
            // 2 : (4) [div.cellClass1, div.cellClass1, div.cellClass1, div.cellClass1]
            // 3 : (4) [div.cellClass1, div.cellClass1, div.cellClass1, div.cellClass1]
            // length: 4

            // for (var j = 0; i < this.cells[i].length; j++) {
            //     wrapper1.appendChild(this.cells[i][j]);
            //     // this.cells[i][j].onclick = function(e) {
            //     //     game.handleOfCellClick(e.target);
            //     // };
    ...
    handleOfCellClick: function(elemInProcess) {
        if (this.turnerState == true || this.turnerState == undefined) {
            ...
            elemInProcess.style.backgroundImage =  this.backgrounder5;
            elemInProcess.dataset.value = true;
            elemInProcess.innerText = "dataset.value = " + elemInProcess.dataset.value;
            elemInProcess.innerText += "\r кликнул на \r" + "cells[" + elemInProcess.dataset.i + "][" + elemInProcess.dataset.j + "]" + "\r";
            ...
            elemInProcess.style.backgroundPosition = "center center";
            elemInProcess.style.backgroundRepeat = "no-repeat";
            elemInProcess.style.backgroundImage = this.backgrounder6;
    ...
    searchOfWinner: function() {
        ...
        for (var ii = 0; ii < x.length; ii++) {
            for (var jjj = 0; jjj < x[ii].length; jjj++) {
                ...
                if (controltrue == x[ii].length) {
                    this.showWinnerLine(x[ii]);
                    return;
                }
                if (controlfalse == x[ii].length) {
                    this.showWinnerLine(x[ii]);
                    return;
                }
        ...
        for (var i2 = 0, j2 = x[i2].length - 1; i2 < x.length; i2++, j2--) {
            ...
            controlArray.push(x[i2][j2]);
            if (controltrueDiag == x[i2].length) {
                this.showWinnerLine(controlArray);
                return;
            }
            if (controlfalseDiag == x[i2].length) {
                this.showWinnerLine(controlArray);
                return;
            }
            if (i2 == x[i2].length - 1) {
        ...
        for (var i3 = 0, j3 = 0; i3 < x.length; i3++, j3++) {
            if (x[i3][j3].dataset.value == "true") controltrueDiag++;
            if (x[i3][j3].dataset.value == "false") controlfalseDiag++;
            controlArrayLeft.push(x[i3][j3]);
            if (controltrueDiag == x[i3].length) {
                this.showWinnerLine(controlArrayLeft);
                return;
            }
            if (controlfalseDiag == x[i3].length) {
                this.showWinnerLine(controlArrayLeft);
                return;
            }
            if (i3 == x[i3].length - 1) {
    ...
    showWinnerLine: function(winnerArrayOfObjects) {
        for (var win = 0; win < winnerArrayOfObjects.length; win++) {
            ...
            winnerArrayOfObjects[win].style.opacity = "1";
...
debugger;
        ...
        //
        // var deleted = document.getElementById("startBlock");
        // if (deleted) {
        //     deleted.innerHTML = "";
        //     deleted.parentNode.removeChild(deleted);
        // }
        ...
        function myFunction() {
            ...
            for (var i = 0, len = game.cells.length; i < len; i++) {
                //cells[i] - это массив, вложенный в ячейку основного массива
                //if (j == 0) {        element.style.clear = "both";        }
                for (var j = 0, len2 = game.cells[i].length; j < len2; j++) {
                    game.cells[i][j]= null;
                }
            }
    ...
    backgrounder5:"url(data:image/png;base64,)",



    backgrounder6:"url(data:image/png;base64,)",
