...
searchOfWinner: function () {
    ...
    //
    //
    //     for (var jj = 0; jj < x[kk].length; jj++) {
    //
    //         console.log("  "+"  "+"x[" + kk + "][" + jj + "] = " + x[kk][jj].dataset.value)+"  ";  }
    //
    //
    //
    // }
    ...
    // console.log(x);
    // console.log(x[0][0].dataset.value);
    // console.log(x[0][1].dataset.value);
    // console.log(typeof x[0][0].dataset.value);
...
// alert ("сейчас i = " + i);
        ...
        for (var jjj = 0; jjj < x[ii].length; jjj++) {
            ...
            //return нужен, чтоб при найденном совпадении не искала иные совпадения в циклах ниже:
            // например, ОДНИМ ЗАПОЛНЕНИЕМ КЛЕТКИ иногда может свпасть и диагональ, и серединная вертикальная линия: чтоб срабатывало только первое совпадение
...
// alert ("сейчас i = " + i);
        ...
        for (var j7 = 0; j7 < x[i7].length; j7++) {
            ...
            // console.log("controltrue=" + controltrue);
            // console.log("controlfalse=" + controlfalse);
            if (controlEmpty == x.length*x[0].length) { console.log("invoke this.stopGame()"); this.stopGame(); return;}
...
// конец метода  searchOfWinner
 ...
 stopGame: function () {
    ...
    if(!document.getElementById("startBlock")) {
    ...
    document.getElementsByClassName("wrapper1")[0].appendChild(button);
    button.addEventListener("click", myFunction);
     ...
     function myFunction (){
         ...
         var deleted = document.getElementsByClassName("wrapper1")[0];

         deleted.parentNode.removeChild(deleted);

         game.init(3,3);
         game.renderElement(this.cells);
