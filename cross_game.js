...
var game = {
    ...
    init: function (rowsNumber, cellsNumber) {
        var rowsNumber = rowsNumber || 3;
        var cellsNumber = cellsNumber || 3;
        ...
        for (var i = 0; i < rowsNumber; i++) {
            ...
            //хранить значения клеток будут вложенные в них массивы
                    for (var j = 0; j < cellsNumber; j++) {
                        this.cells[i].push({
                            value: null,
                            //вызов функции по созданию элемента
                            dom: this.createElement(i, j)
                        });
                    }
        }
    ...
    createElement: function (i, j) {
        ...
        if (i==0) { element.style.clear = "both"; }
      ...
      if (window.wrapper1) {wrapper1.innerHTML = '';}
       ...
       for (var j = 0; j < this.cells.length; j++) {
           ...
           for (var i = 0; i < this.cells[j].length; i++) {
               wrapper1.appendChild(this.cells[i][j].dom);
               this.cells[i][j].dom.innerText += "\n"+"element.value="+ this.cells[i][j].value;
...
console.dir (game.cells[0][2].dom);
